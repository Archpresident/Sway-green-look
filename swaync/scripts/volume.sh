...
notify_user(){
    ...
    # https://en.wikipedia.org/wiki/Box-drawing_character
     bar=$(seq -s "─" $(($volume/13)) | sed 's/[0-9]//g')
    notify-send -h string:x-canonical-private-synchronous:sys-notify -i  "$(get_icon)" -r 2593 -u low "$(get_volume)" "$bar" | paplay /usr/share/sounds/freedesktop/stereo/audio-volume-change.oga
}
# Increase Volume
inc_volume() {
